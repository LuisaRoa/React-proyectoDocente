...
import { BsFillTrashFill, BsPencil, BsFileEarmarkText, BsSearch } from 'react-icons/bs';
...
class Syllabus extends Component {
  ...
  validarFormulario() { //Función para validar los campos del formulario
    ...
    if (!campo["horasSemestral"]) {
      formularioValido = false;
      error["horasSemestral"] = "Por favor, ingresa el número de horas semestrales.";
    } else {
      if (!Number(campo["horasSemestral"])) {
        formularioValido = false;
        error["horasSemestral"] = "Por favor, ingresa un número valido.";
      }
    }
  ...
  seleccionarSyllabus = (syllabus) => { //Función para capturar los datos del syllabus seleccionado
    this.setState({
      tipoModal: 'actualizar',
      form: {
        id: syllabus.id,
        nombre: syllabus.nombre,
        fecha: syllabus.fecha,
        tipoArchivo: syllabus.tipoArchivo,
        tamaño: syllabus.tamaño,
        semestre: syllabus.semestre,
        programaacademico: {
          prac_id: syllabus.programaacademico.prac_id
        },
        progra: syllabus.programaacademico.prac_id,
        materia: {
          mate_id: syllabus.nucleoTemático.mate_id
        },
        mate: syllabus.nucleoTemático.mate_id,
        grupo: syllabus.grupo,
        periodoAca: syllabus.periodoAca,
        horasSemestral: syllabus.horasSemestral
      }
    })
  }
  ...
  insertArchivos = async () => { //Función para guardar un nuevo syllabus 
    if (this.validarFormulario()) {
      await this.setState({
        edicion: {
          ...
          horasSemestral: this.state.form.horasSemestral,
          materia: {
        ...
        .then(response => {
          this.setState({
            form: {
              ...
              horasSemestral: this.state.edicion.horasSemestral,
              materia: {
              ...
              <CCard style={{ width: '100%', textAlign: 'center' }} >
                <CCardHeader>
                  <CForm className="form-inline" >
                    <h2 >< FcFolder size="50px" /> Syllabus</h2>
                    ...
                    <CCol col="2" className="mb-3 mb-xl-0 text-right">
                      ...
                      <a
                        href='http://res.cloudinary.com/dbvsv8u1s/image/upload/v1630558004/cd3i6nylvffgq7nopg3u.pdf' target="_blank">
                        <font color="#000000">Ver Formato Guía </font>
                      </a>
                      <br />
                      <a
                        href='https://plataforma.ucundinamarca.edu.co/aplicaciones/calidad/download.jsp?file=2021-04-22_6666_1.docx&id=344&tipo=2' >
                      ...
                      'opciones':
                        (item) => (
                          <tr>
                            ...
                            <td>
                              <CButton size="lg" onClick={() => { this.seleccionarSyllabus(item); this.setState({ modalEliminar: true }) }}>
                                <BsFillTrashFill />
                              </CButton>
                            </td>
                          </tr>
        ...
        <Modal isOpen={this.state.modalInsertar}>
          ...
          <ModalBody>
            <CForm encType="multipart/form-data" noValidat>
              ...
              <CFormGroup row>
                <CCol md="3">
                  <CLabel htmlFor="text-input">Nro Horas Semestrales</CLabel>
                </CCol>
                <CCol xs="12" md="9">
                  <CInput name="horasSemestral" className="form-control my-2" onChange={this.handleChange} value={form ? form.horasSemestral : ''} />
                  <span style={{ color: "red" }}>{this.state.error["horasSemestral"]}</span>
                </CCol>
              </CFormGroup>
