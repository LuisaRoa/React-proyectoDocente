...
class CierreAcademico extends Component {
  ...
  peticionGet = () => { //Petición para traer todos los informes de cierre academico de un administrativo
    axios.get('http://localhost:8080/cierreAcademico/listarAdministrativo/' + UserProfile.getId(), { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
  ...
  peticionDelete = () => { //Petición para borrar un informe
    axios.delete('http://localhost:8080/cierreAcademico/delete/' + this.state.form.id, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
  ...
  peticionPut = () => { //Petición para editar un informe
    axios.put('http://localhost:8080/cierreAcademico/editar', this.state.form, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
      this.modalInsertar();
      this.peticionGet();
    })
  }
  ...
  insertArchivos = async () => { //Función para guardar un nuevo informe
    if (this.validarFormulario()) {
      ...
      await axios.post('http://localhost:8080/cierreAcademico/upload', f, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } })
        .then(response => {
          this.setState({
            form: {
              id: response.data.id,
