...
class CierreAcademico extends Component {
  state = {
    data: [],
    tablaData: [],
    busqueda: "",
  ...
  validarFormulario() { //Función para validar los datos del formulario
  ...
  peticionGet = () => { //Petición para traer todos los informes de cierre academico de un administrativo
    axios.get('http://ec2-3-136-234-55.us-east-2.compute.amazonaws.com:8080/cierreAcademico/listarAdministrativo/' + UserProfile.getId(), { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
  ...
  peticionDelete = () => { //Petición para borrar un informe
    axios.delete('http://ec2-3-136-234-55.us-east-2.compute.amazonaws.com:8080/cierreAcademico/delete/' + this.state.form.id, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
  ...
  peticionPut = () => { //Petición para editar un informe
    axios.put('http://ec2-3-136-234-55.us-east-2.compute.amazonaws.com:8080/cierreAcademico/editar', this.state.form, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } }).then(response => {
      this.modalInsertar();
      this.peticionGet();
    })
  }
  ...
  insertArchivos = async () => { //Función para guardar un nuevo informe
    if (this.validarFormulario()) {
      ...
      await axios.post('http://ec2-3-136-234-55.us-east-2.compute.amazonaws.com:8080/cierreAcademico/upload', f, { headers: { Authorization: `Bearer ${sessionStorage.getItem(UserProfile.getToken().TOKEN_NAME)}` } })
        .then(response => {
          this.setState({
            form: {
              id: response.data.id,
              ...
              <CCard style={{ width: '100%', textAlign: 'center' }} >
                <CCardHeader>
                  <CForm className="form-inline" >
                    <h2 >< FcFolder size="50px" /> Informe Cierre Académico</h2>
                    ...
                    <CCol col="2" className="mb-3 mb-xl-0 text-right">
                      ...
                      <a
                        href='http://res.cloudinary.com/dbvsv8u1s/image/upload/v1630986916/grkjlzbpzu3kwiivg13i.pdf' target="_blank">
...
export default CierreAcademico;